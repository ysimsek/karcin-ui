...
var SelectInput = /** @class */ (function (_super) {
    ...
    SelectInput.prototype.render = function () {
        ...
        return React.createElement("div", { className: "karcin-select-input select-" + this.state.randomId + " karcin-input", onClick: function () {
                if (_this.props.type !== "single") {
                    _this.inputFocus();
                }
            } }, React.createElement("div", { className: this.props.margin == true ? "propertygrid-select" : "form-group " + this.props.labelPosition }, this.labelResult(), React.createElement("div", { className: "select-input-result" }, selectInputType, (this.state.showing.multiDrop && this.props.items.length !== this.state.selectedItem.length) ? this.getDropDownItems() : '')));
    };
    ...
    SelectInput.prototype.multiSelectResult = function () {
        var _this = this;
        var returnHtml = React.createElement("div", { className: "multi-select-input " + ((this.state.focusControl.control) ? 'input-focus' : '') }, this.getMultiSelectItem(), React.createElement("input", { type: "text", placeholder: (this.state.selectedItem.val.length <= 0 ? this.props.placeholder !== false ? this.props.placeholder : '' : ''), value: this.state.inputText.value, onKeyDown: function (event) {
    ...
    SelectInput.prototype.getMultiSelectItem = function () {
        ...
        if (this.state.selectedItem.val !== undefined && this.state.selectedItem.val.length > 0) {
            this.state.selectedItem.val.forEach(function (value, index) {
                getSelectItem.push(React.createElement("div", { className: "select-item", key: _this.state.randomId + "-" + index }, React.createElement("span", null, (_this.props.selectedRenderer !== undefined) ? _this.props.selectedRenderer(value) : value[_this.props.value]), React.createElement("i", { className: "close-button", onClick: function () { _this.removeSelectItem(index); } }, React.createElement(FaIcon_1.default, { code: "fa-times" }))));
            });
        }
    ...
    SelectInput.prototype.getDropDownItems = function () {
        ...
        return React.createElement("div", { className: "select-dropdown" }, React.createElement(Scrollbars, { style: { width: '100%', height: '100%' } }, itemsList));
    };
    /**
