...
var Menu = /** @class */ (function (_super) {
    ...
    Menu.prototype.menuLoop = function (getData, key, level, collapse) {
        ...
        newData.forEach(function (value, index) {
            ...
            var menuInHTML = React.createElement("div", { className: "menu-centered" }, (_this.props.renderer !== undefined) ?
                _this.props.renderer(value) : (React.createElement("div", null, titleIcon, " ", title)));
            listMenu.push(React.createElement(reactstrap_1.NavItem, { key: index, className: ((actives) ? 'active' : '') + " " + (value.items !== undefined && value.items.length > 0 ? 'downItem' : '') }, React.createElement("div", { className: "menu-head", onClick: function () {
                    if (!_this.state.hover) {
                        _this.toggleActiveMenu(params, value.items);
                    }
                } }, (value.href !== (undefined && null)) ? React.createElement(reactstrap_1.NavLink, { href: (value.href) ? value.href : "#" }, menuInHTML, menuDropIcon) : React.createElement("a", { className: "nav-link" }, menuInHTML, menuDropIcon)), (value.items !== undefined && value.items.length > 0) ? _this.menuLoop(value.items, keys, level + 1, true) : ''));
        });
        var active = (!this.state.hover) ? this.menuItemActive(key) : false;
        return (collapse ? React.createElement(reactstrap_1.Collapse, { isOpen: active }, React.createElement(reactstrap_1.Nav, null, listMenu)) : React.createElement(reactstrap_1.Nav, null, listMenu));
    };
    /**
    ...
    Menu.prototype.toggleActiveMenu = function (param, items) {
        ...
        if (self.props.onChange !== undefined) {
            var changeMenu = this.state.menuActive.slice(0);
