...
var PropertyGrid = /** @class */ (function (_super) {
    ...
    PropertyGrid.prototype.returnElements = function (fields, values) {
        ...
        fields.map(function (field, idx) {
            if (field.divTitle != undefined) {
                components.push(React.createElement("div", { id: idx.toString(), className: "propertygrid-full", style: { width: "100%" } }, React.createElement(Badge_1.default, { id: idx.toString(), className: "propertygrid-full propertygrid-badge", color: _this.props.titleColor }, field.divTitle)));
                components.push(React.createElement("table", { key: idx, style: { width: "100%" } }, React.createElement("tbody", { id: idx.toString() }, _this.returnFields(field.fields, values))));
            }
        });
        return components;
    };
    PropertyGrid.prototype.returnFields = function (fields, values) {
        ...
        fields.map(function (field, idx) {
            ...
            var display2 = {};
            if (comp.length > 0) {
                componentFields.push(React.createElement("tr", { className: "propertygrid-tr", key: idx }, React.createElement("td", { style: { width: "30%" }, className: "propertygrid-td-first" }, field.label), React.createElement("td", { style: me.state.selectedProp == field.name ? display2 : display, className: "propertygrid-td-second" }, comp), React.createElement("td", { style: me.state.selectedProp == field.name ? display : display2, id: field.name, onClick: function () { me.getLabelActiveToInput(idx, field); } }, _this.getLabelActiveToLabel(field))));
            }
        });
    ...
    PropertyGrid.prototype.provideOfShowLabel = function (field, color, code) {
        return React.createElement("span", null, React.createElement(FaIcon_1.default, { style: color, code: code }), React.createElement("span", { style: { marginLeft: 10 } }, field));
    };
    ...
    PropertyGrid.prototype.getAlert = function (value) {
        return React.createElement("div", null, value.title != undefined ? React.createElement("label", { className: "label-properties" }, value.title) : null, React.createElement(reactstrap_1.Alert, { color: value.color }, value.message));
    };
    /**
     * Change the State
     * @param e
